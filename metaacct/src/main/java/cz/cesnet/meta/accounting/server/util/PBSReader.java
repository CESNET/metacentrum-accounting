...
public class PBSReader {
    ...
    final static Logger log = LoggerFactory.getLogger(PBSReader.class);

    public static List<PBSRecord> readPBSFile(InputStream is, Date limitForStartedJobs) throws IOException, ParseException {
        BufferedReader br = new BufferedReader(new InputStreamReader(is));
        List<PBSRecord> records = new ArrayList<PBSRecord>();
        ...
        DateFormat formatter = new SimpleDateFormat("MM/dd/yyyy HH:mm:ss");
            while (true) {
                ...
                // globalni pocitadlo skutecne vyuzitych CPU, inicializuje se pote co se analyzuje exec_host=... Restuje se u dalsiho jobu
                int usedCPUs = 0;
                for (String s : messageParts) {
                    if (s.startsWith("user=")) {
                    ...
                    } else if (s.startsWith("resc_req_total.mem=")) {
                        //if (s.startsWith("Resource_List.mem=")) {
                        String mem = s.substring(s.indexOf('=') + 1);
                        pbsMessage.setReqMem(StringUtils.parsePbsLogSizeFormat(mem));
                    } else if (s.startsWith("resc_req_total.procs=")) {
                        //if (s.startsWith("Resource_List.ncpus=")) {
                        pbsMessage.setReqNcpus(Long.parseLong(s.substring(s.indexOf('=') + 1)));
                    } else if(s.startsWith("resc_req_total.gpu=")) {
                        pbsMessage.setReqGpus(Integer.valueOf(s.substring(s.indexOf('=') + 1)));
                    } else if (s.startsWith("Resource_List.nodes=")) {
                        ...
                        List<PBSHost> pbsHosts = new ArrayList<PBSHost>();
