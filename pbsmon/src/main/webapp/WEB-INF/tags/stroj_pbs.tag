...
<td class="node ${perunMachine.pbsState}" style="height: ${perunMachine.cpuNum/8}em; padding: 0;">
    <c:choose>
        <c:when test="${empty(perunMachine.pbsName)}">
        ...
        <c:when test="${perunMachine.pbsState=='partialy-free'}">
            <div class="usedwrapper" style="height: ${perunMachine.cpuNum/8}em;">
                <div class="usedbox" style="height: ${perunMachine.usedPercent}%;"></div>
                <s:link style="height: ${perunMachine.cpuNum/8}em;" href="/node/${perunMachine.pbsName}"><c:out value="${perunMachine.pbsName}"/> (${perunMachine.cpuNum}&nbsp;CPU)</s:link>
            </div>
        </c:when>
        <c:otherwise>
            <s:link style="height: ${perunMachine.cpuNum/8}em;" href="/node/${perunMachine.pbsName}"><c:out value="${perunMachine.pbsName}"/> (${perunMachine.cpuNum}&nbsp;CPU)</s:link>
