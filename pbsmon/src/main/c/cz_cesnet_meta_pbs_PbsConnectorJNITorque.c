...
/* helper function */
jobjectArray buildArrayTorque(struct batch_status *bs,JNIEnv *env,jclass infoObj_class,jclass pbsException_class,jmethodID pbsExceptionConstructor,jmethodID internId);

/*
 * cz.cesnet.meta.pbs.PbsConnectorJNI class
 * void loadInfo() 
 */
JNIEXPORT void JNICALL Java_cz_cesnet_meta_pbs_PbsConnectorJNI_loadInfoTorque (JNIEnv *env, jobject this) {
 /*JNI variables*/
 ...
 if(con<0) {
    char *buf;
    asprintf(&buf, "pbs_connect(\"%s\"): Error %d = %s (http://www.adaptivecomputing.com/resources/docs/torque/2-5-9/a.derrorcodes.php) \n",pbs_server,pbs_errno,pbs_geterrmsg(con));
 ...
 nodes = buildArrayTorque(bs,env,node_class,pbsException_class,pbsExceptionConstructor,internId);
 ...
 if( (*env)->ExceptionOccurred(env) != NULL) return;
 /* query the server */
 bs = pbs_statjob(con, "", NULL, NULL);
 /* build array of java objects from response */
 jobs = buildArrayTorque(bs,env,job_class,pbsException_class,pbsExceptionConstructor,internId);
 if(jobs==NULL) return;
 /* PBSPro.jobs field */
 ...
 /* build array of java objects from response */
 queues = buildArrayTorque(bs,env,queue_class,pbsException_class,pbsExceptionConstructor,internId);
 ...
 /* build array of java objects from response */
 servers = buildArrayTorque(bs,env,server_class,pbsException_class,pbsExceptionConstructor,internId);
...
jobjectArray buildArrayTorque(struct batch_status *bs,JNIEnv *env,jclass infoObj_class,jclass pbsException_class,jmethodID pbsExceptionConstructor,jmethodID internId) {
 ...
 for(tmp=bs,i=0;tmp!=NULL;tmp=tmp->next,i++) {
   ...
   for(atp=tmp->attribs;atp!=NULL;atp=atp->next) {
     jstring name,value,nameinterned;
     ...
     /*intern name string*/
     nameinterned = (*env)->CallObjectMethod(env,name,internId);
     if( (*env)->ExceptionOccurred(env) != NULL) return NULL;
     /*put to attribute map*/
     (*env)->CallObjectMethod(env,attrsMap,putId,nameinterned,value);
     if( (*env)->ExceptionOccurred(env) != NULL) return NULL;
     /*  free local refs http://java.sun.com/docs/books/jni/html/refs.html */
     (*env)->DeleteLocalRef(env,name);
     (*env)->DeleteLocalRef(env,nameinterned);
     (*env)->DeleteLocalRef(env,value);
   }
   /*  free local refs http://java.sun.com/docs/books/jni/html/refs.html */
   (*env)->DeleteLocalRef(env,objName); 
   (*env)->DeleteLocalRef(env,obj);
   (*env)->DeleteLocalRef(env,attrsMap);
   (*env)->DeleteLocalRef(env,attrsClass);
 }
